...
export class BattleLog {
	...
	static getReplaySpritePreloads(stepQueue: string[]) {
		const spriteBase = 'https://raw.githubusercontent.com/jrodparker-dev/pokemon-sprites/main/sprites/gen5ani/';
		...
		for (const line of stepQueue) {
			...
			spriteURLs.add(`${spriteBase}${species.spriteid}.gif`);
		}
		return [...spriteURLs];
	}
	...
	static createReplayFile(room: {battle: Battle, id?: string, fragment?: string}) {
		...
		const spritePreloads = BattleLog.getReplaySpritePreloads(battle.stepQueue);
		...
		for (const url of spritePreloads) {
			buf += `<link rel="preload" as="image" href="${url}" />\n`;
		}
		...
		buf += '</div>\n';
		buf += `<script type="application/json" class="battle-sprite-preloads">${BattleLog.escapeHTML(JSON.stringify(spritePreloads))}</script>\n`;
