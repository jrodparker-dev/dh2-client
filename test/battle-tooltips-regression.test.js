...
describe('Battle tooltip regressions', () => {
	...
	it('keeps the client Pokemon for switch tooltips so switched-out forme changes stay visible', () => {
		const battle = createBattle([
			'|init|battle',
			'|player|p1|FOO|169',
			'|player|p2|BAR|265',
			...
			'|tier|[Gen 9] Random Battle',
			'|start',
			'|switch|p1a: Eiscue|Eiscue, L83, F|100/100',
			'|switch|p2a: Gliscor|Gliscor, L77, F|242/242',
			'|switch|p1a: Pikachu|Pikachu, L80, M|100/100',
		]);
		const switchedOut = battle.mySide.pokemon[0];
		switchedOut.addVolatile('formechange', 'Eiscue-Noice');
		battle.myPokemon = [
			{ident: switchedOut.ident, details: switchedOut.details, speciesForme: 'Eiscue', item: '', moveTrack: []},
			{ident: 'p1: Pikachu', details: 'Pikachu, L80, M', speciesForme: 'Pikachu', item: '', moveTrack: []},
		];

		const tooltips = new BattleTooltips(battle);
		let matchedClientPokemon = null;
		tooltips.showPokemonTooltip = (clientPokemon, serverPokemon) => {
			matchedClientPokemon = clientPokemon;
			return '';
		};
		tooltips.placeTooltip = () => true;

		tooltips.showTooltip({dataset: {tooltip: 'switchpokemon|0'}});

		assert.equal(matchedClientPokemon, switchedOut);
	});

	it('preserves formechange state when Illusion breaks and the real Pokemon replaces the disguise', () => {
		const battle = createBattle([
			'|init|battle',
			'|player|p1|FOO|169',
			'|player|p2|BAR|265',
			...
			'|start',
			'|switch|p1a: Leafeon|Leafeon, L83, F|100/100',
			'|switch|p2a: Bombirdier|Bombirdier, L77, F|242/242',
		]);
		const disguised = battle.farSide.active[0];
		disguised.addVolatile('formechange', 'Eiscue-Noice');
		const revealed = battle.farSide.addPokemon('Zoroark', 'p2: Zoroark', 'Zoroark, M');

		battle.farSide.replace(revealed);

		assert.equal(revealed.volatiles.formechange[1], 'Eiscue-Noice');
	});
});
